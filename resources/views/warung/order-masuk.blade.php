            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <div>
                        ...
                        <small class="text-muted">{{ $order->created_at->format('H:i') }} · Konsumen #{{ $order->buyer_id }}</small>
                    </div>
                ...
                <div class="mb-2 d-flex flex-wrap align-items-center gap-1">
                    ...
                    @endif
                    {{-- Cek status pembayaran di cod_settlements --}}
                    @php
                        $settlement = $order->status === 'selesai'
                            ? \Illuminate\Support\Facades\DB::table('cod_settlements')->where('order_id', $order->id)->first()
                            : null;
                    @endphp
                    @if ($settlement)
                        <span class="badge rounded-pill px-2 py-0" style="background-color: var(--warna-aksen-kedua); color: #fff; font-size: 0.65rem;">
